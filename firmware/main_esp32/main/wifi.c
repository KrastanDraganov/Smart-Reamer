...
void wifi_event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
	...
	if (event_id == WIFI_EVENT_AP_STACONNECTED) {
		...
		if (s_retry_num < CFG_WIFI_STA_MAX_TRIES) {
			esp_wifi_connect();
			s_retry_num++;
...
void wifi_init_softap(void) {
	...
	wifi_config_t wifi_config = {
		.ap = {
			.ssid           = CFG_WIFI_SSID,
			.ssid_len       = strlen(CFG_WIFI_SSID),
			.channel        = CFG_WIFI_CHANNEL,
			.password       = CFG_WIFI_PASS,
			.max_connection = CFG_WIFI_MAX_CONN,
			.authmode       = WIFI_AUTH_WPA_WPA2_PSK
		},
	};
	if (strlen(CFG_WIFI_PASS) == 0) {
...
bool wifi_init_sta(void) {
	...
	wifi_config_t wifi_config = {
		.sta = {
			.ssid               = CFG_WIFI_STA_SSID,
			.password           = CFG_WIFI_STA_PASS,
